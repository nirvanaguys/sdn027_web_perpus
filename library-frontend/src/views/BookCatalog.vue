<template>
  <div>
    ...
    <section class="relative overflow-hidden bg-gradient-to-b from-indigo-950 via-slate-900 to-slate-900 text-white pt-14 pb-20 px-4 sm:px-6 lg:px-8">
      <!-- Background Glowing Orbs -->
      <div class="absolute top-0 left-1/4 w-96 h-96 bg-indigo-500/15 rounded-full blur-3xl pointer-events-none"></div>
      <div class="absolute -bottom-10 right-1/4 w-96 h-96 bg-violet-500/15 rounded-full blur-3xl pointer-events-none"></div>

      <div class="relative max-w-5xl mx-auto text-center space-y-6">
        <!-- Badge -->
        <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/15 text-indigo-300 text-xs font-semibold tracking-wide shadow-inner">
          <Sparkles class="w-3.5 h-3.5" />
          <span>Sistem Manajemen Perpustakaan Terpadu</span>
        </div>

        <!-- Headline -->
        <h1 class="text-3xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight leading-tight text-transparent bg-clip-text bg-gradient-to-r from-white via-slate-100 to-indigo-200">
          Temukan & Pinjam Koleksi <br class="hidden sm:block" />Buku Favoritmu
        </h1>

        <!-- Subtitle -->
        <p class="max-w-2xl mx-auto text-slate-300 text-sm sm:text-base leading-relaxed">
          Eksplorasi koleksi literatur, cek posisi rak fisik dan ketersediaan stok secara langsung, lalu pinjam buku hanya dalam satu klik.
        </p>

        <!-- Prominent Search Bar -->
        <div class="max-w-2xl mx-auto pt-2">
          <div class="relative flex items-center bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-1.5 shadow-2xl focus-within:ring-2 focus-within:ring-indigo-400 focus-within:border-transparent transition-all">
        ...
        <!-- Metric Highlights -->
        <div class="flex flex-wrap items-center justify-center gap-6 pt-4 text-xs sm:text-sm text-slate-300">
          <div class="flex items-center gap-2">
            <span class="w-2 h-2 rounded-full bg-emerald-400"></span>
            <span>Total <strong>{{ allBooks.length }}</strong> Judul Terdaftar</span>
          </div>
          <div class="flex items-center gap-2">
            <span class="w-2 h-2 rounded-full bg-indigo-400"></span>
            <span><strong>{{ availableBooksCount }}</strong> Buku Siap Dipinjam</span>
          </div>
          <div class="flex items-center gap-2">
            <span class="w-2 h-2 rounded-full bg-amber-400"></span>
            <span><strong>{{ uniqueShelvesCount }}</strong> Lokasi Rak Fisik</span>
          </div>
        </div>
      </div>
    </section>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 relative z-10 pb-16">
      <!-- Filter Bar -->
      <div class="bg-white rounded-2xl p-4 sm:p-5 shadow-sm border border-slate-200/80 mb-8 flex flex-col sm:flex-row items-center justify-between gap-4">
        <!-- Stock Filter Tabs -->
      ...
      >
        ...
        <p class="text-sm text-slate-500 mt-1 max-w-sm mx-auto">
          Tidak ada koleksi yang cocok dengan filter atau kata kunci "{{ searchQuery }}".
        </p>
        <button
          @click="resetFilters"
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        ...
        >
          ...
          >
            <!-- Decorative circle -->
            <div class="absolute -right-6 -bottom-6 w-24 h-24 rounded-full bg-white/10 blur-sm pointer-events-none"></div>

            <div class="flex items-start justify-between z-10">
              <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-black/25 backdrop-blur-md">
              ...
              >
                ...
                <span>{{ book.stock > 0 ? `${book.stock} Ada` : 'Habis' }}</span>
              </span>
            </div>

            <!-- Stylized Spine Typography -->
            <div class="z-10 flex items-center justify-between">
              <span class="font-mono text-[11px] text-white/80 bg-black/20 px-2 py-0.5 rounded-md">
                ISBN: {{ book.isbn }}
              </span>
            </div>
          </div>
          ...
          <div class="p-5 flex-1 flex flex-col">
            <div class="flex items-center gap-1.5 mb-2.5">
              <span class="inline-flex items-center px-2.5 py-0.5 rounded-lg text-[11px] font-bold bg-indigo-50 text-indigo-700">
                {{ book.category || 'Umum' }}
              </span>
            </div>
            ...
            <div class="mt-3 space-y-1.5 text-xs text-slate-500">
              <div class="flex items-center gap-2">
                ...
                <span class="truncate">Penulis: <strong class="text-slate-700 font-medium">{{ book.author }}</strong></span>
              </div>
              <div class="flex items-center gap-2">
      ...
      >
        <div class="bg-white rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden border border-slate-100">
          ...
          >
            ...
            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-black/30 mb-3">
              <MapPin class="w-3.5 h-3.5" />
              Rak: {{ selectedDetailBook.shelf_location }}
            </span>
            <h2 class="text-xl font-bold leading-snug">{{ selectedDetailBook.title }}</h2>
            <p class="text-sm text-white/80 mt-1">Karya {{ selectedDetailBook.author }}</p>
          </div>
          ...
          <div class="p-6 space-y-4">
            <div class="grid grid-cols-2 gap-3 p-4 rounded-2xl bg-slate-50 border border-slate-100 text-xs">
              <div>
                ...
                <strong class="text-slate-800 text-sm font-semibold">{{ selectedDetailBook.category || 'Umum' }}</strong>
              </div>
              <div>
                <span class="text-slate-400 block mb-0.5">Penerbit</span>
                <strong class="text-slate-800 text-sm font-semibold">{{ selectedDetailBook.publisher }}</strong>
              </div>
              <div>
                <span class="text-slate-400 block mb-0.5">Kode ISBN</span>
                <strong class="font-mono text-slate-800 text-sm font-semibold">{{ selectedDetailBook.isbn }}</strong>
              </div>
              <div>
                <span class="text-slate-400 block mb-0.5">Stok Fisik</span>
                <span
                  class="font-bold inline-block px-2 py-0.5 rounded-md"
                  :class="selectedDetailBook.stock > 0 ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'"
                >
              ...
              <div class="col-span-2">
                ...
                <strong class="text-slate-800 text-sm font-semibold">7 Hari Kalender</strong>
              </div>
            </div>

            <div class="text-xs text-slate-500 leading-relaxed bg-indigo-50/60 p-3.5 rounded-xl border border-indigo-100/80">
              💡 <strong>Ketentuan Sirkulasi:</strong> Anggota dapat meminjam buku fisik ini langsung di rak perpustakaan. Pengembalian dapat dilakukan melalui menu <em>Buku Saya</em>.
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-2">
              ...
              <button
                ...
                :class="selectedDetailBook.stock > 0 
                  ? 'bg-indigo-600 hover:bg-indigo-700' 
                  : 'bg-slate-200 text-slate-400 cursor-not-allowed'"
              >
                <BookOpen class="w-4 h-4" />
                <span>{{ selectedDetailBook.stock > 0 ? 'Pinjam Buku Ini' : 'Stok Tidak Tersedia' }}</span>
              </button>
            </div>
          </div>
        </div>
...
const availableBooksCount = computed(() => allBooks.value.filter((b) => b.stock > 0).length);
...
const filteredBooks = computed(() => {
  return allBooks.value.filter((book) => {
    ...
    if (selectedCategory.value && book.category !== selectedCategory.value) return false;
    if (selectedShelf.value && book.shelf_location !== selectedShelf.value) return false;
    return true;
  });
});
...
const askBorrowBook = (book) => {
  if (!authStore.isAuthenticated()) {
    toastStore.warning('Silakan masuk (login) terlebih dahulu untuk meminjam buku.');
...
const executeBorrow = async () => {
  ...
  try {
    const res = await api.post('/transactions/borrow', { book_id: bookToBorrow.value.id });
