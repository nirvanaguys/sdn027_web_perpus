<template>
  <header class="sticky top-0 z-40 backdrop-blur-xl bg-white/85 border-b border-slate-200/80 transition-all">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-20">
        
        <!-- Brand Logo -->
        <router-link to="/" class="flex items-center gap-3 group">
          <div class="w-11 h-11 rounded-2xl bg-gradient-to-tr from-indigo-600 via-indigo-700 to-violet-600 flex items-center justify-center text-white shadow-lg shadow-indigo-500/25 group-hover:scale-105 group-hover:shadow-indigo-500/40 transition-all duration-300">
            <BookOpen class="w-6 h-6 stroke-[2.2]" />
          </div>
          <div>
            <span class="text-xl font-extrabold tracking-tight bg-gradient-to-r from-slate-900 via-indigo-950 to-indigo-800 bg-clip-text text-transparent">
              Pustaka<span class="text-indigo-600">Ku</span>
            </span>
            <span class="hidden sm:block text-[11px] font-medium text-slate-400 tracking-wider uppercase">
              Perpustakaan Pintar
            </span>
          </div>
        </router-link>
        ...
        <nav class="hidden md:flex items-center gap-1.5 bg-slate-100/70 p-1.5 rounded-2xl border border-slate-200/60">
          <router-link
            to="/"
        ...
        <div class="hidden md:flex items-center gap-3">
          <template v-if="authStore.isAuthenticated()">
            <div class="flex items-center gap-3 pl-2 pr-3 py-1.5 rounded-2xl bg-slate-50 border border-slate-200/80">
              <div
                class="w-9 h-9 rounded-xl flex items-center justify-center font-bold text-xs uppercase text-white shadow-xs"
                :class="authStore.isAdmin() ? 'bg-gradient-to-tr from-amber-500 to-orange-500' : 'bg-gradient-to-tr from-indigo-500 to-violet-500'"
              >
                {{ getUserInitials() }}
              </div>
              <div class="text-left">
    ...
    >
      <div v-if="mobileMenuOpen" class="md:hidden border-t border-slate-200 bg-white/95 backdrop-blur-xl px-4 pt-3 pb-6 space-y-3">
        <router-link
          @click="mobileMenuOpen = false"
          to="/"
          class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-semibold transition"
          :class="$route.path === '/' ? 'bg-indigo-50 text-indigo-600' : 'text-slate-700 hover:bg-slate-50'"
        >
        ...
        <router-link
          ...
          to="/my-loans"
          class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-semibold transition"
          :class="$route.path === '/my-loans' ? 'bg-indigo-50 text-indigo-600' : 'text-slate-700 hover:bg-slate-50'"
        >
        ...
        <div class="pt-3 border-t border-slate-100">
          <template v-if="authStore.isAuthenticated()">
            <div class="flex items-center justify-between px-3 py-2 bg-slate-50 rounded-xl mb-3">
              <div>
                <p class="text-xs text-slate-500">Login sebagai</p>
                <p class="text-sm font-bold text-slate-900">{{ authStore.user?.name }}</p>
              </div>
              <span class="text-xs px-2 py-0.5 rounded-md font-semibold bg-indigo-100 text-indigo-700 uppercase">
                {{ authStore.user?.role }}
              </span>
            </div>
            <button
...
const fetchLoansCount = async () => {
  if (authStore.isAuthenticated() && !authStore.isAdmin()) {
    try {
      ...
      activeLoansCount.value = loans.filter((l) => l.status === 'borrowed').length;
    } catch {
